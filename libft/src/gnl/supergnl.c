...
/*   Updated: 2025/11/19 22:34:07 by thblack-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	buffer[FD_SIZE][BUF_SIZE + 1];
	char		*tmp;
	char		*line;
	ssize_t		size;

	line = ft_strjoin("", "");
	if (fd < 0 || fd >= FD_MAX || FD_SIZE <= 0 || BUF_SIZE <= 0 || !line)
		return (safe_exit(line, NULL));
	while (1)
	{
		tmp = ft_strchr(buffer[fd], '\n');
		if (tmp)
			return (ft_find_line(&line, buffer[fd], tmp));
		line = ft_join_free(line, buffer[fd]);
		if (!line)
			return (NULL);
		buffer[fd][0] = '\0';
		size = read(fd, buffer[fd], BUF_SIZE);
		if (size < 0)
			return (safe_exit(line, buffer[fd]));
		buffer[fd][size] = '\0';
		if (size == 0)
